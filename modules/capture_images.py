...
def capture(input_video):
    user_id = input("Enter Your ID (numbers only): ")
    name = input("Enter Your Name (alphabets only): ")

    # if "user_id is a number" and "name consists of alphabetic chars only" then
    if is_number(user_id) and name.isalpha():
        ...
        while True:
            ...
            if ret is True:  # video is detected
                ...
                for(x, y, w, h) in faces:
                    ...
                    cv2.imwrite("training_images" + os.sep + name + "." + user_id + '.' +
                                str(increment_num) + ".jpg", img[y:y+h, x:x+w])  # ##gray[y:y+h, x:x+w]

                    # display the resulting frame
                    cv2.imshow('Capturing Face - Intrusion Detection System', img)  # ##gray
        ...
        row = [user_id, name]
        with open("files" + os.sep + "user_details.csv", 'a+') as csv_file:
            writer = csv.writer(csv_file)
            writer.writerow(row)
        csv_file.close()

    # else Invalid Input ("user_id is not a number" or "name doesn't contain only alphabetic chars")
    else:
        # if user_id input is correct then
